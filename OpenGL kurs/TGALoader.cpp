...
void MessageBox(void* garbage, const char*msg,const char* type)
{
    std::cout<<"["<<type<<"] "<<msg<<"\n";
}
...
bool LoadTGA(Texture * texture, char * filename)				// Load a TGA file
{
	...
	{
		MessageBox(NULL, "Could not open texture file", "ERROR");	// Display an error message
		return false;														// Exit function
	}

	if(fread(&tgaheader, sizeof(TGAHeader), 1, fTGA) == 0)					// Attempt to read 12 byte header from file
	{
		MessageBox(NULL, "Could not read file header", "ERROR");		// If it fails, display an error message 
	...
	{
		MessageBox(NULL, "TGA file be type 2 or type 10 ", "Invalid Image");	// Display an error
		fclose(fTGA);
		return false;																// Exit function
	}
...
{																			// TGA Loading code nehe.gamedev.net)
	...
	{										
		MessageBox(NULL, "Could not read info header", "ERROR");		// Display error
	...
	{
		MessageBox(NULL, "Invalid texture information", "ERROR");	// Display Error
	...
	{
		MessageBox(NULL, "Could not allocate memory for image", "ERROR");	// Display Error
	...
	{
		MessageBox(NULL, "Could not read image data", "ERROR");		// Display Error
...
{ 
	...
	{
		MessageBox(NULL, "Invalid texture information", "ERROR");	// If it isnt...Display error
	...
	do
	{
		...
		{
			MessageBox(NULL, "Could not read RLE header", "ERROR");	// Display Error
			if(fTGA != NULL)												// If file is open
			{
				fclose(fTGA);												// Close file
			}
		...
		{																	// that follow the header
			...
			for(short counter = 0; counter < chunkheader; counter++)		// Read RAW color values
			{
				...
				{
					MessageBox(NULL, "Could not read image data", "ERROR");		// IF we cant, display an error
				...
				{
					MessageBox(NULL, "Too many pixels read", "ERROR");			// if there is too many... Display an error!
		...
		else																			// chunkheader > 128 RLE data, next color reapeated chunkheader - 127 times
		{
			...
			{	
				MessageBox(NULL, "Could not read from file", "ERROR");			// If attempt fails.. Display error (again)
